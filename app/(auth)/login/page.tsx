...
export default function LoginPage() {
  ...
  const getLoginErrorMessage = (err: any): string => {
    ...
    if (status === 403 || detail?.toLowerCase().includes('deactivat') || detail?.toLowerCase().includes('disabled')) {
    ...
    <>
      <div className="min-h-screen flex" style={{ background: '#0a0f1e', fontFamily: 'Inter, sans-serif' }}>
        ...
        <div
          ...
          style={{ background: 'linear-gradient(135deg, #0d1627 0%, #0a0f1e 100%)', borderRight: '1px solid rgba(255,255,255,0.06)' }}
        >
          ...
          <div className="flex gap-6">
            {[
              { value: '84%', label: 'Avg. Readiness Score' },
              { value: '50M+', label: 'Data Points Analyzed' },
              { value: 'Top 12%', label: 'Candidate Ranking' },
            ].map(s => (
              <div key={s.label} className="glass-card p-4 text-center" style={{ flex: 1 }}>
                <div className="text-2xl font-bold mb-1" style={{ color: '#00c4cc' }}>{s.value}</div>
        ...
        <div className="flex flex-1 items-center justify-center p-8">
          <div className="w-full max-w-md">
            ...
            <p className="text-sm mb-8" style={{ color: '#64748b' }}>
              {isLogin ? 'Sign in to continue your career journey' : 'Get started with FuturePath AI for free'}
            </p>
